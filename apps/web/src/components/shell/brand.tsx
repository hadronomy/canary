...
function Brand(props: { compact?: boolean }) {
  return (
    <div className="flex h-10 min-w-0 items-center gap-3 overflow-hidden">
      <div className="grid size-10 shrink-0 place-items-center rounded-[0.8rem] bg-foreground text-background shadow-[0_0_0_1px_oklch(1_0_0_/_8%)]">
        <LightningIcon className="size-5" weight="fill" />
      </div>
      <div
        aria-hidden={props.compact}
        className={cn(
          'min-w-0 transition-[opacity,transform,filter] duration-150 ease-[var(--ease-out-strong)] motion-reduce:transition-none',
