...
export class AxiomErrorReporter extends Effect.Service<AxiomErrorReporter>()(
	"AxiomErrorReporter",
	{
		accessors: true,
		effect: Effect.gen(function* () {
			const config = yield* Config.all({
				...
				serviceName: Config.string("AXIOM_SERVICE_NAME").pipe(
					Config.withDefault("canary-server"),
				),
				serviceVersion: Config.string("AXIOM_SERVICE_VERSION").pipe(Config.option),
				environment: Config.string("AXIOM_ENVIRONMENT").pipe(
				...
				Effect.tryPromise({
					try: async () => {
						const response = await fetch(
							`${config.url}/v1/datasets/${logsDataset}/ingest`,
							{
								method: "POST",
								headers: {
